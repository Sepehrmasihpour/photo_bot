...
#     global is_active  # Access the global is_active flag to check bot status
#     if is_active:  # Ensure bot is active and message is from the test group
#         try:
#             await context.bot.send_message(chat_id=TEST_GROUP_ID, text=text)
#         except Exception as e:
#             logging.error(
...
if __name__ == "__main__":
    ...
    photo_handler = MessageHandler(
        filters.PHOTO & (~filters.FORWARDED) & (~filters.COMMAND), photo_message_handler
    )
    music_handler = MessageHandler(
        filters.AUDIO & (~filters.COMMAND), music_message_handler
    )
    video_handler = MessageHandler(
        filters.VIDEO & (~filters.FORWARDED) & (~filters.COMMAND), video_message_handler
    )
    # send_message_handle = MessageHandler(send_message)
    ...
    # application.add_handler(send_message_handle)
