...
@app.post("/sendMessage/text/mainGroup/{message}")
async def message_group(text: str):
    result = await message_group_via_bot(text)
    if "error" in result:
        raise HTTPException(status_code=400, detail=result["error"])
    return {
...
@app.post("/post/text/mainChannel/{post}")
...
@app.post("sendMessage/photo/{photo}/{chat_id}/{caption}")
